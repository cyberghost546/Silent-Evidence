...
export const metadata: Metadata = {
  ...
  description: 'Top horror story writers ranked by reader likes. Who reigns supreme in the darkness?',
};
...
async function fetchRanked(premiumOnly: boolean) {
  const authors = await prisma.user.findMany({
    ...
    include: {
      ...
      stories:  { where: { status: 'PUBLISHED' }, select: { views: true, _count: { select: { likes: true } } } },
...
const medalTone = ['text-yellow-400', 'text-gray-300', 'text-amber-600'];
  ...
  return (
    <main className="min-h-screen bg-gray-900 text-white">
      ...
      {/* Hero */}
      <div className={`relative border-b border-gray-800 py-12 overflow-hidden ${isElite ? 'bg-yellow-950/30' : 'bg-gray-950'}`}>
        <div className={`absolute inset-0 ${isElite
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    {author.stories} {author.stories === 1 ? 'story' : 'stories'} · {author.followers} followers
                  </p>
                </div>
                ...
                <div className="flex items-center gap-5 text-sm text-gray-400 shrink-0">
                  <div className="text-center">
                    <p className={`font-bold ${isElite ? 'text-yellow-300' : 'text-white'}`}>{author.totalLikes}</p>
        ...
        {isElite && (
          <p className="text-center text-xs text-gray-600 mt-8">
            Only Horror Elite subscribers appear here. <Link href="/premium" className="text-yellow-600 hover:text-yellow-500 transition">Join Elite →</Link>
          </p>
        )}
      </div>
