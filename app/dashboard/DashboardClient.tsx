...
type DashboardData = {
  ...
  topStories: TopStory[];
  viewsChart: ChartDay[];
  readsPerDay: DayPoint[];
  followersPerDay: DayPoint[];
  readsChange: number | null;
...
function ChangeBadge({ pct }: { pct: number | null }) {
  ...
  return (
    <span
      className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${up ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}
    >
      {up ? '+' : ''}
      {pct}%
    </span>
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 hover:border-gray-700 transition group">
      <div className="flex items-start justify-between mb-3">
        ...
        {spark && (
          <Sparkline data={spark} color={textColor.replace('text-', '#').replace('-400', '')} />
        )}
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          },
          {
            ...
            value: data.thisMonthReads,
            icon: <IconEye />,
            color: 'text-gray-300',
          },
          {
            label: 'New followers',
            value: data.thisMonthFollowers,
            icon: <IconUsers />,
            color: 'text-gray-300',
          },
          {
            ...
            value: streak,
            icon: <IconFire />,
            color: streak > 0 ? 'text-orange-400' : 'text-gray-600',
          },
        ...
        <Card title="Top Stories" sub="by views">
          <ResponsiveContainer width="100%" height={data.topStories.length * 44}>
            ...
            >
              ...
              <Tooltip
                {...tooltipStyle}
                formatter={((v: number) => [v.toLocaleString(), 'Views']) as any}
              />
          ...
          <div className="mt-4 pt-4 border-t border-gray-800 space-y-2">
            {data.topStories.map((s, i) => (
              <div key={s.id} className="flex items-center gap-3">
                ...
                <Link
                  href={`/story/${s.slug}`}
                  className="flex-1 min-w-0 text-sm text-gray-300 hover:text-white transition truncate"
                >
                  {s.title}
                </Link>
      ...
      <Card
        ...
        sub="on your stories"
        action={
          <Link href="/my-stories" className="text-xs text-gray-600 hover:text-gray-400 transition">
            View all
          </Link>
        }
      >
              ...
              <div key={c.id} className="py-3 first:pt-0 last:pb-0">
                <div className="flex items-center gap-2 mb-1">
                  <img
                    src={
                      c.avatar ??
                      `https://ui-avatars.com/api/?name=${encodeURIComponent(c.username)}&background=dc2626&color=fff&size=32`
                    }
                    alt={c.username}
                  ...
                  <Link
                    ...
                    className="text-[10px] text-gray-500 hover:text-gray-300 transition truncate max-w-[120px]"
                  >
                    {c.storyTitle}
                  </Link>
            ...
            {data.recentTips.map((tip) => (
              ...
              >
                <div>
                  ...
                  {tip.message && (
                    <p className="text-xs text-gray-500 italic mt-0.5">"{tip.message}"</p>
                  )}
