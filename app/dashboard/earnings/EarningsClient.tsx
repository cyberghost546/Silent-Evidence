...
interface Earnings {
  ...
  net: number; fee: number; feeBps: number; paidOut: number; available: number;
...
export default function EarningsClient() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('connect')) {
      fetch('/api/author/connect').then(() => load()).catch(() => {});
    }
  }, [load]);

  const startOnboarding = async () => {
    setBusy(true); setMsg('');
    const res = await fetch('/api/author/connect', { method: 'POST', headers: { 'x-csrf-token': await getCsrfToken() } });
    setBusy(false);
    if (res.ok) { const d = await res.json(); window.location.href = d.url; }
    else { const d = await res.json().catch(() => ({})); setMsg(d.error ?? 'Could not start setup.'); }
  };

  const withdraw = async () => {
    setBusy(true); setMsg('');
    const res = await fetch('/api/author/payout', { method: 'POST', headers: { 'x-csrf-token': await getCsrfToken() } });
    setBusy(false);
    const d = await res.json().catch(() => ({}));
    if (res.ok) { setMsg(`Withdrew ${money(d.amountCents)}.`); load(); }
  ...
  if (!data) return <p className="text-gray-500 text-sm py-12 text-center">Could not load earnings.</p>;
        ...
        <div className="space-y-2">
          {sources.map(([label, gross, count]) => (
            <div key={label} className="flex items-center justify-between text-sm">
              <span className="text-gray-400">{label} <span className="text-gray-600">· {count}</span></span>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        ...
        {!data.payoutsConfigured ? (
          <p className="text-xs text-gray-500">
            Your earnings are being tracked. Withdrawals open once the site finishes
            connecting its payment provider.
          </p>
        ) : !data.connect.onboarded ? (
          <>
            <p className="text-xs text-gray-500 mb-3">
              Set up payouts with Stripe to withdraw your balance. You&apos;ll be taken to
              Stripe to add your details securely.
            </p>
          ...
          <>
            <p className="text-xs text-gray-500 mb-3">Payouts are set up. Withdraw your available balance to your bank via Stripe.</p>
            <button type="button" onClick={withdraw} disabled={busy || e.available < 1000}
              className="px-4 py-2 text-xs font-semibold bg-red-600 hover:bg-red-700 text-white rounded-lg transition disabled:opacity-50">
              {busy ? 'Processing…' : `Withdraw ${money(e.available)}`}
            </button>
            {e.available < 1000 && <p className="text-[11px] text-gray-600 mt-2">Minimum withdrawal is $10.00.</p>}
          </>
        )}
          ...
          <div className="space-y-1.5">
            {data.payouts.map((p) => (
              <div key={p.id} className="flex items-center justify-between text-xs">
                ...
                <span className={p.status === 'paid' ? 'text-green-400' : p.status === 'failed' ? 'text-red-400' : 'text-amber-400'}>{p.status}</span>
              </div>
            ))}
          </div>
        </div>
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className={`rounded-xl p-4 border ${accent ? 'bg-red-950/20 border-red-900/40' : 'bg-gray-900 border-gray-800'}`}>
      <p className="text-[11px] uppercase tracking-wider text-gray-500">{label}</p>
      <p className={`text-2xl font-extrabold mt-1 ${accent ? 'text-red-400' : 'text-white'}`}>{value}</p>
    </div>
  );
}
