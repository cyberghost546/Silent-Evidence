...
// Feed showing the latest stories from authors the logged-in user follows.
// Loads client-side so we can do infinite scroll without a full page reload.
  ...
  const fetchStories = useCallback(async (skip: number, append: boolean) => {
    try {
      ...
      if (append) {
        ...
        setStories(batch);
        setIsEmpty(batch.length === 0 && (data.followedIds?.length ?? 0) === 0);
      }
  ...
  return (
    <main className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        ...
        {!notLoggedIn && !loading && stories.length === 0 && !isEmpty && (
          <div className="text-center py-20">
            ...
            {stories.map(story => {
              ...
              return (
                <Link
                  ...
                  className="group bg-gray-800 border border-gray-700 hover:border-red-600/60 rounded-xl overflow-hidden transition-all duration-300 shadow-[0_4px_20px_rgba(220,38,38,0.1)] hover:shadow-[0_8px_30px_rgba(220,38,38,0.35)] flex flex-col"
                >
