...
export async function POST(req: Request) {
  ...
  if (action === 'warn') {
    await prisma.userWarning.create({
      data: { userId, adminId, reason: reason || 'Policy violation' },
    });
    await prisma.auditLog.create({
      data: { adminId, action: 'WARN_USER', targetId: userId, targetType: 'User', detail: reason },
    });
    ...
    await prisma.auditLog.create({
      data: {
        ...
        targetId: userId,
        targetType: 'User',
        detail: `Suspended for ${suspendDays} days. Reason: ${reason}`,
      },
    });
  } else if (action === 'ban') {
    await prisma.user.update({ where: { id: userId }, data: { isBanned: true } });
    await prisma.auditLog.create({
      data: { adminId, action: 'BAN_USER', targetId: userId, targetType: 'User', detail: reason },
    });
  } else if (action === 'unban') {
    await prisma.user.update({
      where: { id: userId },
      data: { isBanned: false, suspendedUntil: null },
    });
    await prisma.auditLog.create({
      data: { adminId, action: 'UNBAN_USER', targetId: userId, targetType: 'User', detail: reason },
    });
  }
