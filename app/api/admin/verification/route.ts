...
export async function POST(req: Request) {
  ...
  const requests: { userId: number; status: string }[] = setting?.value
    ? JSON.parse(setting.value)
    : [];

  const updated = requests.map((r) =>
    r.userId === userId ? { ...r, status: action === 'approve' ? 'approved' : 'rejected' } : r
  );
  ...
  await prisma.auditLog.create({
    data: {
      ...
      targetId: userId,
      targetType: 'User',
      detail: `${action === 'approve' ? 'Approved' : 'Rejected'} verification for user #${userId}`,
    },
  });
