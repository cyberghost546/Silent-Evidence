...
export async function GET() {
  ...
  const stories = await prisma.story.findMany({
    ...
    orderBy: { views: 'desc' },
    select:  { id: true, title: true, slug: true, views: true, createdAt: true, content: true,
                _count: { select: { likes: true, comments: true } } },
  });
  const storyIds = stories.map(s => s.id);

  const [
    followerCount, draftCount, scheduledCount,
    recentReads7, recentReads30,
    prevReads30,                  // 60–30 days ago for % change
    recentFollows30, prevFollows30,
    tips, recentTips,
    recentComments,
    thisMonthPublished,
    thisMonthFollowers,
    ...
    storyIds.length
      ? prisma.readingHistory.findMany({ where: { storyId: { in: storyIds }, readAt: { gte: sevenDaysAgo } }, select: { readAt: true } })
      : Promise.resolve([]),

    storyIds.length
      ? prisma.readingHistory.findMany({ where: { storyId: { in: storyIds }, readAt: { gte: thirtyDaysAgo } }, select: { readAt: true } })
      : Promise.resolve([]),

    // Previous 30-day window for % change comparison
    storyIds.length
      ? prisma.readingHistory.count({ where: { storyId: { in: storyIds }, readAt: { gte: sixtyDaysAgo, lt: thirtyDaysAgo } } })
      : Promise.resolve(0),

    prisma.follow.findMany({ where: { followingId: userId, createdAt: { gte: thirtyDaysAgo } }, select: { createdAt: true } }),
    prisma.follow.count({ where: { followingId: userId, createdAt: { gte: sixtyDaysAgo, lt: thirtyDaysAgo } } }),

    prisma.tip.aggregate({ where: { toUserId: userId }, _sum: { amount: true } }),
    prisma.tip.findMany({
      where: { toUserId: userId }, orderBy: { createdAt: 'desc' }, take: 5,
      select: { id: true, amount: true, message: true, createdAt: true, from: { select: { username: true } } },
    }),
    ...
    prisma.story.count({ where: { authorId: userId, status: 'PUBLISHED', createdAt: { gte: startOfMonth } } }),
  ...
  const readsChange     = prevReads30   > 0 ? Math.round(((recentReads30.length - prevReads30)   / prevReads30)   * 100) : null;
  const followersChange = prevFollows30 > 0 ? Math.round(((recentFollows30.length - prevFollows30) / prevFollows30) * 100) : null;

  // Best single day (reads) from the 7-day window
  const bestDayReads = Math.max(...Object.values(day7Counts), 0);

  return NextResponse.json({
    totalViews, totalLikes, totalComments, followerCount,
    draftCount, scheduledCount, publishedCount: stories.length,
    tipEarningsCents: tips._sum.amount ?? 0,
    recentTips: recentTips.map(t => ({
      id: t.id, amount: t.amount, message: t.message,
      createdAt: t.createdAt.toISOString(), fromUsername: t.from.username,
    })),
    topStories: stories.slice(0, 5),
    viewsChart, readsPerDay, followersPerDay,
    // New fields
    readsChange, followersChange,
    thisMonthPublished, thisMonthFollowers, thisMonthReads,
    bestDayReads,
    recentComments: recentComments.map((c: any) => ({
      id: c.id, content: c.content,
      createdAt: c.createdAt.toISOString(),
      storyTitle: c.story.title, storySlug: c.story.slug,
