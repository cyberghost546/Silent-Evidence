...
export default function VerificationClient({ requests: initial }: { requests: Request[] }) {
  ...
  const act = async (userId: number, action: 'approve' | 'reject') => {
    ...
    if (res.ok) {
      ...
      setRequests((r) =>
        r.map((x) =>
          x.userId === userId ? { ...x, status: action === 'approve' ? 'approved' : 'rejected' } : x
        )
      );
          ...
          <div className="mt-4 space-y-2">
            {decided.map((r) => (
              <div key={r.userId} className="flex items-center justify-between text-sm">
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded-full border font-medium ${r.status === 'approved' ? 'bg-green-500/10 text-green-400 border-green-500/30' : 'bg-gray-500/10 text-gray-400 border-gray-500/30'}`}
                >
                  {r.status}
                </span>
              </div>
