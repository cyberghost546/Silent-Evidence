  ...
  return (
    <div>
      ...
      <p className="text-gray-500 text-sm mb-8">
        {stories.length} stor{stories.length === 1 ? 'y' : 'ies'} queued for future publication.
      </p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs text-gray-500 uppercase tracking-wider border-b border-gray-800">
                ...
                <th className="px-5 py-3">Publishes</th>
                <th className="px-5 py-3 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-800">
              {stories.map((s) => {
                const timeLeft = s.scheduledAt
                  ? Math.round((s.scheduledAt.getTime() - Date.now()) / 3600000)
                  : null;
                return (
                    ...
                    <td className="px-5 py-3">
                      ...
                      {timeLeft !== null && (
                        <p className="text-xs text-gray-600">
                          {timeLeft > 0 ? `in ${timeLeft}h` : 'overdue'}
                        </p>
                      )}
                    </td>
