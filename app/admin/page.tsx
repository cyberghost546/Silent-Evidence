...
export default async function AdminOverviewPage() {
  ...
  const [recentUsers, recentStories] = await Promise.all([
    prisma.user.findMany({ orderBy: { createdAt: 'desc' }, take: 5, select: { id: true, username: true, role: true, createdAt: true } }),
    prisma.story.findMany({ orderBy: { createdAt: 'desc' }, take: 5, include: { author: { select: { username: true } } } }),
  ]);

  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-1">Overview</h1>
      <p className="text-gray-500 text-sm mb-8">Welcome back. Here's what's happening on the site.</p>

      {/* ── Stat cards — each is a clickable link to the relevant admin section ── */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        <StatCard label="Total Users"       value={userCount}      icon={<Users className="w-6 h-6" />}        href="/admin/users"   />
        <StatCard label="Total Stories"     value={storyCount}     icon={<BookOpen className="w-6 h-6" />}     href="/admin/stories" />
        <StatCard label="Published"         value={publishedCount} icon={<BookOpen className="w-6 h-6" />}     href="/admin/stories" />
        <StatCard label="Comments"          value={commentCount}   icon={<MessageCircle className="w-6 h-6" />} href="#"              />
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 mb-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href="/admin/analytics" className="text-xs text-red-400 hover:text-red-300 transition">Full analytics →</Link>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/admin/users" className="text-xs text-red-400 hover:text-red-300 transition">View all →</Link>
          </div>
          <div className="space-y-3">
            {recentUsers.map((u) => (
              <div key={u.id} className="flex items-center justify-between">
                ...
                <span className="text-xs text-gray-600">{new Date(u.createdAt).toLocaleDateString()}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/admin/stories" className="text-xs text-red-400 hover:text-red-300 transition">View all →</Link>
          </div>
          <div className="space-y-3">
            {recentStories.map((s) => (
...
function StatCard({ label, value, icon, href }: { label: string; value: number; icon: ReactNode; href: string }) {
  return (
    <Link href={href} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 hover:border-red-600/40 transition group">
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`flex-shrink-0 text-xs px-2 py-0.5 rounded-full border font-medium ${styles[status] ?? styles.DRAFT}`}>
      {status.toLowerCase()}
    </span>
  );
}
