...
export default function FeaturedAuthorsClient({ featured: initial }: { featured: Author[] }) {
  ...
  const add = async (author: Author) => {
    if (featured.length >= 6) {
      flash('Max 6 featured authors.');
      return;
    }
    if (featured.find((f) => f.id === author.id)) {
    ...
    setResults([]);
    setSearch('');
    await fetch('/api/admin/featured-authors', {
      method: 'POST',
  ...
  return (
    <div className="space-y-6">
              ...
              <div key={u.id} className="flex items-center justify-between px-4 py-3">
                <div>
                  <p className="text-sm font-semibold text-white">
                    {u.username}
                    {u.isVerified && <span className="text-blue-400 ml-1">✓</span>}
                  </p>
                  <p className="text-xs text-gray-500">
                    {u.stories} stories · {u.followers} followers
                  </p>
                </div>
                <button
                  onClick={() => add(u)}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
        ...
        {featured.length === 0 ? (
          <p className="text-gray-600 text-sm">No featured authors set.</p>
        ) : (
              ...
              <div key={a.id} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-white">
                      {a.username}
                      {a.isVerified && <span className="text-blue-400 ml-1 text-xs">✓</span>}
                    </p>
                    <p className="text-xs text-gray-500">
                      {a.stories} stories · {a.followers} followers
                    </p>
                  </div>
                </div>
                <button
