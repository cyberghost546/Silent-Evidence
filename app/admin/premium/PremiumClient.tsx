...
export default function PremiumClient({ initialSubs, initialCounts }: Props) {
  ...
  async function grantPremium() {
    ...
    const search = await fetch(
      `/api/admin/user-support?q=${encodeURIComponent(grantUsername.trim())}`
    );
    if (!search.ok) {
    ...
    const res = await fetch('/api/admin/premium', {
      ...
      body: JSON.stringify({
        userId: user.id,
        plan: grantPlan,
        currentPeriodEnd: grantExpiry || null,
      }),
    });
    const data = await res.json();
    setGrantLoading(false);
                ...
                {displayed.map((sub) =>
                  editing?.id === sub.id ? (
                    ...
                    <tr key={sub.id} className="hover:bg-gray-800/40 transition">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${STATUS_STYLES[sub.status] ?? STATUS_STYLES.inactive}`}
                        >
