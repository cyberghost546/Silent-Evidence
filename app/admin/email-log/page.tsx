  ...
  return (
    <div>
      ...
      <p className="text-gray-500 text-sm mb-8">
        Every email the system has sent — useful for debugging delivery issues.
      </p>
      ...
      <div className="flex gap-2 flex-wrap mb-4">
        {[
          { label: 'All', value: 'all' },
          ...types.map((t) => ({ label: `${t.type} (${t._count.type})`, value: t.type })),
        ].map((opt) => (
          <a
            key={opt.value}
            href={`/admin/email-log?type=${opt.value}`}
          ...
          <tbody className="divide-y divide-gray-800">
            {logs.length === 0 && (
              <tr>
                <td colSpan={5} className="px-5 py-10 text-center text-gray-600">
                  No emails logged yet. Email logging starts once the system sends its first email
                  after this update.
                </td>
              </tr>
            )}
            {logs.map((l) => (
              <tr key={l.id} className="hover:bg-gray-800/40 transition">
                ...
                <td className="px-5 py-3">
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full border font-medium ${l.status === 'sent' ? 'bg-green-500/10 text-green-400 border-green-500/30' : 'bg-red-500/10 text-red-400 border-red-500/30'}`}
                  >
                    {l.status}
                  </span>
                </td>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2 mt-6">
          {page > 1 && (
            <a
              href={`/admin/email-log?page=${page - 1}${typeFilter ? `&type=${typeFilter}` : ''}`}
