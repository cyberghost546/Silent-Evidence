...
export default function PollsClient({ polls: initial }: { polls: Poll[] }) {
  ...
  const create = async () => {
    ...
    if (!question.trim() || validOpts.length < 2) {
      flash('Need a question and at least 2 options.');
      return;
    }
    setCreating(true);
  ...
  const close = async (id: number) => {
    await fetch('/api/admin/polls', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id, endsAt: new Date().toISOString() }),
    });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
        ...
        <div className="flex gap-3 mb-4">
          <button onClick={addOption} className="text-xs text-gray-400 hover:text-white transition">
            + Add option
          </button>
        </div>
        <div className="mb-4">
      ...
      {polls.map((p) => {
        ...
        return (
          <div
            key={p.id}
            className={`bg-gray-900 border rounded-2xl p-6 ${closed ? 'border-gray-800' : 'border-red-600/20'}`}
          >
