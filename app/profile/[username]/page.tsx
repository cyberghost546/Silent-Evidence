...
export default async function ProfilePage({ params }: Props) {
  ...
  const isFollowing =
    loggedInId && !isOwnProfile
      ? !!(await prisma.follow.findFirst({
          where: { followerId: loggedInId, followingId: user.id },
        }))
      : false;
  ...
  const pinnedStory = user.pinnedStoryId
    ? (user.stories.find((s) => s.id === user.pinnedStoryId) ?? null)
    : null;
  // Exclude the pinned story from the main grid so it doesn't appear twice
  const unpinnedStories = pinnedStory
    ? user.stories.filter((s) => s.id !== pinnedStory.id)
    : user.stories;
        ...
        <div className="lg:grid lg:grid-cols-[260px_1fr] lg:gap-10 mb-16">
          ...
          <aside className="mb-8 lg:mb-0">
            <div className="lg:sticky lg:top-24 space-y-5">
              {/* Bio */}
              {user.profile?.bio && (
                <p className="text-sm text-gray-400 leading-relaxed">{user.profile.bio}</p>
              )}
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 space-y-3">
                  ...
                  {favMood && (
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wide border ${moodMeta(favMood).color}`}
                      >
                ...
                >
                  {pinnedStory.coverImage ? (
                    ...
                    <div className="w-20 h-20 rounded-xl bg-gradient-to-br from-red-950 to-gray-800 shrink-0 flex items-center justify-center">
                      <svg
                        className="w-7 h-7 text-red-600/50"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={1.5}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <h3 className="text-sm font-semibold text-white group-hover:text-red-400 transition mt-0.5 line-clamp-2 leading-snug">
                      {pinnedStory.title}
                    </h3>
                    {pinnedStory.excerpt && (
            ...
            {user.stories.length === 0 ? (
              <div className="text-center py-20 border border-dashed border-gray-800 rounded-2xl bg-gray-900/40">
                ...
                <p className="text-xs text-gray-600 mt-1.5">
                  {isOwnProfile
                    ? 'Share your first dark tale with the world.'
