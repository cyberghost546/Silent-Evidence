...
export default async function YouMightAlsoFear({ currentStoryId, mood, categoryId }: Props) {
  ...
  // Build query params — hints are used for the logged-out fallback
  const params = new URLSearchParams({
    exclude:  String(currentStoryId),
    take:     '4',
    ...(mood       ? { mood }                    : {}),
  ...
  // Fetch recommendations server-side, forwarding the userId cookie so the
  // API can identify the current user and personalise results
  let stories: RecommendedStory[] = [];
  try {
    const res = await fetch(`${baseUrl}/api/recommendations?${params}`, {
      headers: { cookie: cookieHeader },
      // Revalidate every 5 minutes — recs don't need to be real-time
      next: { revalidate: 300 },
    });
