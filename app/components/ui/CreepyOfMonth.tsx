...
export default async function PickOfMonth() {
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 pb-12">
      ...
      <div className="relative rounded-2xl overflow-hidden border border-red-600/30 bg-gray-800 shadow-[0_4px_20px_rgba(124,58,237,0.15)] hover:shadow-[0_8px_30px_rgba(124,58,237,0.4)] transition-all duration-300">
        ...
        <div className="absolute top-4 right-4 flex items-center gap-1.5 bg-red-600 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg">
          👑 Pick of the Month
        </div>
        ...
        <div className="relative p-8 md:p-10 flex flex-col gap-4 max-w-2xl">
          ...
          <Link href={`/category/${story.category.slug}`} className="text-xs font-bold uppercase tracking-widest text-red-400 hover:text-red-300 transition w-fit">
            {story.category.name}
          </Link>

          {/* Story title — links to the full story page */}
          <Link href={`/story/${story.slug}`}>
            <h3 className="text-2xl md:text-3xl font-bold text-white hover:text-red-300 transition leading-tight">
              {story.title}
            </h3>
          </Link>
          ...
          <div className="flex items-center gap-3 mt-1">
            <img src={avatar} alt={story.author.username} className="w-8 h-8 rounded-full border-2 border-red-500/40 object-cover" />
          ...
          <Link
            ...
            className="mt-2 w-fit bg-red-600 hover:bg-red-700 text-white text-sm font-semibold px-5 py-2.5 rounded-lg transition"
          >
            Read Story →
          </Link>
        </div>
