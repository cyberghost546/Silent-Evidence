    ...
    return (
      <div className="rounded-xl border border-yellow-500/20 bg-yellow-500/5 p-5 text-center">
        ...
        <p className="text-xs text-gray-500 mb-4">
          Unlock custom profile themes and animated avatar borders with a premium subscription.
        </p>
        <a
          href="/premium"
          className="inline-block px-4 py-2 bg-yellow-500 hover:bg-yellow-400 text-black text-xs font-bold rounded-xl transition"
        >
          Upgrade to Premium
        </a>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {(Object.entries(BORDERS) as [BorderKey, (typeof BORDERS)[BorderKey]][]).map(
            ([key, b]) => (
              <button
                key={key}
                type="button"
      ...
      <div className="flex items-center gap-4 p-4 bg-gray-800/40 rounded-xl border border-gray-700">
        <div className="relative w-16 h-16 shrink-0">
          <div
            className={`absolute -inset-1 rounded-full bg-linear-to-br ${THEMES[theme].glow} blur-md ${BORDERS[border].cssClass}`}
          />
          <div className="relative w-16 h-16 rounded-full bg-gray-700 border-4 border-gray-950 flex items-center justify-center text-2xl"></div>
        </div>
        <div>
          <p className="text-sm font-semibold text-white">Preview</p>
          <p className={`text-xs mt-0.5 ${THEMES[theme].accent}`}>
            {THEMES[theme].name} · {BORDERS[border].name}
          </p>
        </div>
      </div>
