...
const QUESTIONS = [
  {
    q: 'In "The Shining", what does Jack Torrance type over and over?',
    options: [
      "Here's Johnny",
  ...
  {
    q: 'Which horror film features a villain who can only be killed if you say his name five times in a mirror?',
    options: ['Candyman', 'Bloody Mary', 'The Ring', 'Sinister'],
    answer: 0,
  },
  {
    q: 'What is the name of the demon in "The Exorcist"?',
    options: ['Pazuzu', 'Astaroth', 'Belial', 'Belphegor'],
    answer: 0,
  },
  {
    q: 'In "Halloween" (1978), what is Michael Myers\'s original Halloween mask made from?',
    options: [
      'A clown mask',
  ...
  {
    q: 'What creature hunts by sound in "A Quiet Place"?',
    options: ['Blind cave monsters', 'Telepathic aliens', 'Sound-sensitive creatures', 'Vampires'],
    answer: 2,
  },
  {
    q: 'In "Get Out", what is the hypnotic state victims are put in called?',
    options: ['The Abyss', 'The Sunken Place', 'The Deep', 'The Void'],
    answer: 1,
  },
  {
    q: 'Which author wrote "It", "The Shining", and "Pet Sematary"?',
    options: ['Dean Koontz', 'H.P. Lovecraft', 'Stephen King', 'Clive Barker'],
    answer: 2,
  },
  {
    q: 'What is the name of the videotape that kills viewers 7 days after watching in "The Ring"?',
    options: ['Ringu', "Samara's Tape", 'It has no name', 'The Cursed Tape'],
    answer: 2,
  },
  {
    q: 'In "Hereditary", what ancient demon is the family being groomed to host?',
    options: ['Paimon', 'Malphas', 'Barbatos', 'Zagan'],
    answer: 0,
  },
  {
    q: 'Which H.P. Lovecraft creation is described as a cosmic entity sleeping under the Pacific Ocean?',
    options: ['Nyarlathotep', 'Cthulhu', 'Shub-Niggurath', 'Azathoth'],
    answer: 1,
  },
];
    ...
    return (
      <div className="bg-gray-800 border border-gray-700 rounded-2xl p-8 text-center">
        ...
        <p className="text-gray-400 mb-6">
          10 questions covering horror films, books, and lore. No time limit.
        </p>
        {/* Start button */}
        <button
  ...
  if (phase === 'result')
    return (
      <div className="bg-gray-800 border border-gray-700 rounded-2xl p-8 text-center">
        ...
        {!isLoggedIn && (
          <p className="text-xs text-gray-500 mb-4">Log in to save your score to the leaderboard</p>
        )}
        ...
        <div className="flex flex-col gap-3 mb-6">
          {QUESTIONS.map((q, i) => (
            <div
              key={i}
              className={`flex items-start gap-3 text-left p-3 rounded-xl text-sm ${answers[i] === q.answer ? 'bg-green-500/10 border border-green-500/20' : 'bg-green-500/10 border border-green-500/20'}`}
            >
              ...
              <div>
                ...
                {answers[i] !== q.answer && (
                  <p className="text-xs text-green-400">
                    You answered: {q.options[answers[i] ?? 0]}
                  </p>
                )}
              </div>
            </div>
        ...
        <button
          onClick={restart}
          className="px-6 py-2.5 bg-gray-700 hover:bg-gray-600 text-white text-sm font-semibold rounded-lg transition"
        >
          Try Again
        </button>
      </div>
          ...
          return (
            <button
              ...
              className={`w-full text-left px-5 py-3.5 rounded-xl border text-sm font-medium transition ${
                isCorrect
                  ? 'bg-green-500/20 border-green-500 text-green-300' // green = correct
                  : isWrong
                    ? 'bg-red-500/20 border-red-500 text-red-300' // red = wrong pick
                    : isSelected
                      ? 'bg-gray-700 border-gray-500 text-white' // selected but not yet revealed
            ...
            >
              ...
              <span className="mr-3 text-gray-500">{['A', 'B', 'C', 'D'][i]}.</span>
              {opt}
            </button>
          );
        })}
