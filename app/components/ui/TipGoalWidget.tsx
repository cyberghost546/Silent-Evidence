  ...
  if (!data && isAuthor) {
    return (
      <div className="border border-dashed border-gray-700 rounded-xl p-4 text-center">
        ...
        {editing && <GoalForm {...{ goalTitle, setGoalTitle, goalAmount, setGoalAmount, goalDesc, setGoalDesc, saving, onSave: handleSave, onCancel: () => setEditing(false) }} />}
      </div>
    );
  }
  ...
  return (
    <div className="border border-yellow-500/20 bg-yellow-500/5 rounded-xl p-5 my-6">
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-yellow-400 mb-0.5">Tip Goal</p>
        ...
        {isAuthor && !editing && (
          <button onClick={() => setEditing(true)} className="text-xs text-gray-500 hover:text-gray-300 transition flex-shrink-0">
            Edit
          </button>
        )}
      </div>
      ...
      <div className="flex justify-between text-xs text-gray-400">
        <span><span className="text-white font-semibold">${raised}</span> raised</span>
        <span>{percent}% of <span className="text-white font-semibold">${goal}</span> goal</span>
      </div>

      {/* Edit form */}
      {editing && (
        <GoalForm
          {...{ goalTitle, setGoalTitle, goalAmount, setGoalAmount, goalDesc, setGoalDesc, saving, onSave: handleSave, onCancel: () => setEditing(false) }}
...
function GoalForm({ goalTitle, setGoalTitle, goalAmount, setGoalAmount, goalDesc, setGoalDesc, saving, onSave, onCancel, onDelete }: {
  goalTitle: string; setGoalTitle: (v: string) => void;
  goalAmount: string; setGoalAmount: (v: string) => void;
  ...
  return (
    <div className="mt-4 space-y-3 border-t border-gray-700 pt-4" suppressHydrationWarning>
      <input suppressHydrationWarning value={goalTitle} onChange={e => setGoalTitle(e.target.value)}
      ...
      <input suppressHydrationWarning type="number" value={goalAmount} onChange={e => setGoalAmount(e.target.value)}
        placeholder="Goal amount in USD (e.g. 50)"
        className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-600 transition" />
      <textarea suppressHydrationWarning value={goalDesc} onChange={e => setGoalDesc(e.target.value)}
