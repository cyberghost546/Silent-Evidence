...
export default function StoryActionsDropdown({
  ...
  const shareX = () =>
    window.open(
      `https://twitter.com/intent/tweet?text=${encodeURIComponent(storyTitle)}&url=${encodeURIComponent(window.location.href)}`,
      '_blank'
    );
  ...
  const shareWhatsApp = () =>
    window.open(
      `https://wa.me/?text=${encodeURIComponent(storyTitle + ' ' + window.location.href)}`,
      '_blank'
    );
  ...
  const shareReddit = () =>
    window.open(
      `https://reddit.com/submit?title=${encodeURIComponent(storyTitle)}&url=${encodeURIComponent(window.location.href)}`,
      '_blank'
    );
  ...
  const report = async () => {
    ...
    try {
      ...
      /* show feedback regardless of network error */
    }
    setReporting(false);
    setReported(true);
  };
