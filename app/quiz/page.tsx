...
export default async function QuizPage() {
  ...
  const topScores = await prisma.quizAttempt
    .findMany({
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              10 questions. How well do you know horror?
            </p>
          </div>
        </div>
          ...
          <div className="mt-12">
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
              {topScores.map((attempt, i) => (
                <div
                  key={attempt.id}
                  className={`flex items-center gap-4 px-5 py-3 ${i < topScores.length - 1 ? 'border-b border-gray-700' : ''}`}
                >
                  <span
                    className={`text-sm font-bold w-6 text-center ${i === 0 ? 'text-yellow-400' : i === 1 ? 'text-gray-400' : i === 2 ? 'text-orange-600' : 'text-gray-600'}`}
                  >
                    {`${i + 1}`}
                  </span>
                  <Link
                    href={`/user/${attempt.user.username}`}
                    className="text-sm text-white hover:text-red-400 transition flex-1"
                  >
                    {attempt.user.username}
                  </Link>
                  <span className="text-sm font-bold text-white">
                    {attempt.score}
                    <span className="text-gray-500 font-normal">/{attempt.total}</span>
                  </span>
