...
vi.mock('next/headers', () => ({ cookies: async () => ({ get: (n:string)=>(jar[n]!==undefined?{value:jar[n]}:undefined) }) }));
...
async function cookieFor(id: number, v: number) { jar.userId=String(id); jar.userId_v=String(v); jar.userId_sig=await sign(id,v); }

let id: number;
beforeAll(async () => { id = (await prisma.user.create({ data:{username:'zz_ev',email:'zz_ev@x.invalid',password:'x'.repeat(60)}, select:{id:true} })).id; });
afterAll(async () => { await prisma.user.delete({where:{id}}).catch(()=>{}); await prisma.$disconnect(); });
