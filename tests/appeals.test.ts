...
beforeAll(async () => {
  ...
  noTfaAdmin = (
    await prisma.user.create({
      data: {
        username: 'zz_ap_no2fa',
        email: 'zz_ap_no2fa@x.invalid',
        password: 'x'.repeat(60),
        role: 'ADMIN',
...
const setUser = async (id: number) => {
  ...
  jar.userId_v = '0';
  jar.userId_sig = await signVersioned(id, 0);
};
...
describe('DSA appeals lifecycle', () => {
  it('modA records an action; user sees it and appeals; modA cannot review own; modB overturns', async () => {
    ...
    const rec = await recordAction(
      jsonReq({
        ...
        reason: 'HATE_SPEECH',
        explanation: 'Removed for a slur in paragraph two.',
      })
    );
    ...
    const notif = await prisma.notification.findFirst({
      where: { userId: affected, type: 'MODERATION' },
    });
    ...
    const filed = await fileAppeal(
      jsonReq({ actionId, message: 'The word was in dialogue by a villain, not hate speech.' })
    );
    expect(filed.status).toBe(201);

    // 2b. A second appeal on the same action is refused.
    const dup = await fileAppeal(
      jsonReq({ actionId, message: 'appealing again to test the guard' })
    );
    expect(dup.status).toBe(409);

    const appeal = await prisma.moderationAppeal.findFirst({
      where: { actionId },
      select: { id: true },
    });
    ...
    const selfReview = await decide(patchReq({ decision: 'UPHELD' }), {
      params: Promise.resolve({ id: String(appealId) }),
    });
    expect(selfReview.status).toBe(403);

    // 4. Admin B overturns it.
    await setUser(modB);
    const overturn = await decide(
      patchReq({ decision: 'OVERTURNED', note: 'Agreed — it was in-character dialogue.' }),
      { params: Promise.resolve({ id: String(appealId) }) }
    );
    expect(overturn.status).toBe(200);

    // 5. Effects: appeal OVERTURNED, action REVERSED, user notified.
    const finalAppeal = await prisma.moderationAppeal.findUnique({
      where: { id: appealId },
      select: { status: true, reviewerId: true },
    });
    expect(finalAppeal!.status).toBe('OVERTURNED');
    expect(finalAppeal!.reviewerId).toBe(modB);
    const finalAction = await prisma.moderationAction.findUnique({
      where: { id: actionId },
      select: { status: true },
    });
    ...
    const again = await decide(patchReq({ decision: 'UPHELD' }), {
      params: Promise.resolve({ id: String(appealId) }),
    });
  ...
  it('a user cannot appeal a decision that is not theirs', async () => {
    ...
    const res = await fileAppeal(
      jsonReq({ actionId, message: 'trying to appeal someone elses action' })
    );
