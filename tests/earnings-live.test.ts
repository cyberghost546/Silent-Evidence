...
let authorId: number, buyerId: number, storyId: number, chapterId: number, bundleId: number, catId: number;
...
beforeAll(async () => {
  ...
  const bundle = await prisma.storyBundle.create({ data: { title: 'zz bundle', slug: `zz-b-${Date.now()}`, price: 500, authorId }, select: { id: true } });
  bundleId = bundle.id;
...
describe('getEarnings (live)', () => {
  it('sums all four sources, applies the 10% fee, and tracks payouts', async () => {
    ...
    await prisma.payout.create({ data: { authorId, amountCents: 1000, status: 'paid', coveredThrough: new Date() } });
    ...
    await prisma.payout.create({ data: { authorId, amountCents: 500, status: 'pending', coveredThrough: new Date() } });
    e = await getEarnings(authorId);
    expect(e.paidOut).toBe(1000);
  }, 60000);
});
